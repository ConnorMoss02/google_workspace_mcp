...
def build_label_color(
    ...
    current_color: Optional[Mapping[str, str]] = None,
) -> Optional[Dict[str, str]]:
    """Build the `color` object for a Gmail label, or None when no color is given.

    `current_color` is the color already on the label, so updating one half of a
    pair keeps the other half instead of dropping it.
    """
    if background_color is None and text_color is None:
        return None

    color: Dict[str, str] = dict(current_color or {})
    if background_color is not None:
        color["backgroundColor"] = _validate_label_color(
            "background_color", background_color
        )
    if text_color is not None:
        color["textColor"] = _validate_label_color("text_color", text_color)
    return color
