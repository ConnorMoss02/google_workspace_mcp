...
async def _read_saved(result: str) -> str:
    """Read an exported message without blocking the async test event loop."""
    content = await asyncio.to_thread(Path(_saved_path(result)).read_bytes)
...
async def test_full_export_raw_saves_complete_eml(stdio_storage):
    ...
    assert await _read_saved(result) == raw_mime
...
async def test_full_export_text_uses_plaintext(stdio_storage):
    ...
    assert await _read_saved(result) == "Plain body"
...
async def test_full_export_html_saves_raw_html(stdio_storage):
    ...
    assert await _read_saved(result) == "<p><b>Rich HTML</b></p>"
...
async def test_full_export_preserves_edge_whitespace(stdio_storage):
    ...
    assert await _read_saved(html_result) == html_with_edges
    ...
    assert await _read_saved(text_result) == text_with_edges
...
async def test_full_export_does_not_truncate(stdio_storage):
    ...
    saved = await _read_saved(result)
...
async def test_full_export_text_converts_html_when_no_plaintext(stdio_storage):
    ...
    assert await _read_saved(result) == "Converted body"
...
async def test_full_export_html_falls_back_to_text_and_labels_it(stdio_storage):
    ...
    assert await _read_saved(result) == "Only plaintext here"
